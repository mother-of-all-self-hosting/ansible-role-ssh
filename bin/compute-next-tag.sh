...
# published. It is at v1.0.2-1.
#
# This role is unusual among the roles of this fleet in that it deploys no
# software: it configures the OpenSSH server that the host already runs. There
# is no container image, no upstream project and therefore no version of its
# own. `defaults/main.yml` holds nothing a version could be read out of - the
# numbers in it are ports, timeouts and retry counts - and Renovate has no leaf
# here to bump. The `1.0.2` in the tags is a number somebody picked by hand.
#
...
# of the order in which pull requests land, and lets any change to the role -
# a bugfix, a new sshd directive, a default that was wrong - release itself
# without a human tagging it.
