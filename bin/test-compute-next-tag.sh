...
# that has already moved past `-0`.
#
# The defaults file deliberately carries the traps this role's real one has:
# numbers that a script looking for something version-shaped could latch onto.
# `system_security_ssh_port: 22` would become `v22-0`, `system_security_ssh_max_auth_tries: 3`
# would become `v3-0` and `system_security_ssh_client_alive_interval: 120` would
# become `v120-0`; none of those is a version of anything and the tag must not be
...
scenario() {
	...
	mkdir -p "$workdir/bin" "$workdir/defaults" "$workdir/meta" "$workdir/tasks" "$workdir/templates/etc/ssh" "$workdir/molecule/default" "$workdir/.github/workflows"
	...
	cat > defaults/main.yml <<-'YAML'
		...
		system_security_ssh_login_grace_time: 30
		system_security_ssh_per_source_max_startups: 4
...
edit_defaults="printf 'system_security_ssh_extra: false\n' >> defaults/main.yml"
edit_task="printf 'a task\n' >> tasks/main.yml"
edit_template="printf 'AllowAgentForwarding no\n' >> templates/etc/ssh/sshd_config.j2"
...
expect 'defaults edit' v1.0.2-2 "$(merge "$edit_defaults")"
expect 'task edit'     v1.0.2-3 "$(merge "$edit_task")"
expect 'template edit' v1.0.2-4 "$(merge "$edit_template")"
expect 'meta edit'     v1.0.2-5 "$(merge "$edit_meta")"
...
# A script grepping for something version-shaped would answer v22-0, v3-0 or
# v120-0 here.
scenario 'A defaults change is a release, not a new series'
expect 'defaults edit' v1.0.2-2 "$(merge "$edit_defaults")"
expect 'defaults edit' v1.0.2-3 "$(merge "$edit_defaults")"

# Being a host-level role with no image of its own, most of the work in this
# repository is test and CI scaffolding. None of it changes what a playbook run
# does to a host, so none of it is worth a tag that every consumer then has to
# bump past.
scenario 'Commits that do not affect the role'
expect 'README'       ''         "$(merge "$edit_readme")"
expect 'a workflow'   ''         "$(merge "$edit_workflow")"
expect 'the suite'    ''         "$(merge "$edit_molecule")"
expect 'a script'     ''         "$(merge "$edit_script")"
expect 'a task'       v1.0.2-2   "$(merge "$edit_task")"

scenario 'Release numbers past 9'
for release_number in 2 3 4 5 6 7 8 9 10; do
	git tag "v1.0.2-$release_number"
done
expect 'a task' v1.0.2-11 "$(merge "$edit_task")"

# Older series exist and must not be continued from. If the newest tag were
# picked lexically rather than by version, `v1.0.10-0` would lose to `v1.0.2-1`
# and the series would run backwards.
...
# `v10.0.0-0` would lose to `v9.0.0-0`, and `v1.0.10-0` would lose to `v1.0.2-1`.
scenario 'A double-digit major series'
git tag v9.0.0-0
git tag v10.0.0-0
expect 'a task' v10.0.0-1 "$(merge "$edit_task")"

scenario 'A double-digit patch component'
git tag v1.0.10-0
expect 'a task' v1.0.10-1 "$(merge "$edit_task")"
...
# up `v1.2` or `v1.0.3-rc1` would derail the series; `v3.0.0` without a release
...
git tag -d v1.0.0-0 v1.0.1-0 v1.0.2-0 v1.0.2-1 > /dev/null
printf 'a task\n' >> tasks/main.yml
git add -A
git commit -qm 'Merge'
